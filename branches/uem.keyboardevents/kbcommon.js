...
// THe maps array assigns the primary keymap, an index into the arrays in the
// the secondary maps.  The primary map defines the basic key code to key
// identifier mapping for a layout type.  The secondary maps describe the
// small deviations that a given layout makes with respect to the basic
// mapping in the primary map.  There are secondary maps that describe changes,
// deletions, and additions to the primary map.
UEMKB.maps = [
...
if (document.createEventObject) {
  ...
  // For keyboardEvent purposes, treat SF as IE
  // UEMKB.browser = "SF";
  UEMKB.browser = "IE";
}
...
// Map of keycode to key identifier for control keys.
// There are Opera browser bugs to work around.
if (UEMKB.os == "OP") {
  UEMKB.kc2iKeyboard = {
    // Keypad /
    "47":"U+002F",
...
UEMKB.invkctoi = function() {
  ...
  var stillin = 1;
  var ret = new Object();
  for (kc in UEMKB.kc2iKeyboard) {
    value = UEMKB.kc2iKeyboard[kc];
    ret[value] = kc;
  }
 ...
 * and secondary add map.  There is no automatic way to reliably detect the
 * layout that applies to the current window.
 * 
 * A list of the layouts is available by iterating over the members of the
 * associative array UEMKB.ix.
 * <pre>
 * for (var k1 in UEMKB.ix) {
 *   layouts[layouts.length] = k1;
 * }
 * layouts.sort();
 * </pre>
 */
...
/**
 * Map key code to key identifier.  This is a utility function used by
 * browser-specific code in the UEMKB library.
...
UEMKB.kctoi = function(keycode) {
  ...
  var xmap = UEMKB.addMap[kcs];
  if (!xmap) {
    xmap = UEMKB.chgMap[kcs];
    if (!xmap) {
      xmap = UEMKB.primaryMap[kcs];
      if (!xmap) {
        xmap = UEMKB.kc2iCommon[kcs];
        if (!xmap) {
          xmap = UEMKB.kc2iKeyboard[kcs];
          if (!xmap) {
            xmap = "U+0000";
          }
        }
